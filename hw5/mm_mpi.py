...
def create_submatrices(matrix, k):
    n = (len(matrix)//k)
    #print("n", n, "k", k)
    submatrices = [[0 for i in range(n)] for j in range(k)]
    for i in range(0, k):
        for j in range(0, n):
            #print("i", i, "j", j)
            submatrices[i][j] = matrix[i * n + j]
...
def multiply_rows(matrix):
    ...
    rb = len(matrix)
    cb = len(matrix[0])
    ra = len(transpose_matrix)
    ca = len(transpose_matrix[0])

    partial_matrix = [[0 for i in range(ra)] for j in range(cb)]

    for i in range(ra):
        for j in range(cb):
            for k in range(rb):
                partial_matrix[i][j] += transpose_matrix[i][k] * matrix[k][j]
...
def sum_partial_matrices(part_mat_a, part_mat_b):
    ...
    res = [[0 for i in range(len(part_mat_a))] for j in range(len(part_mat_a[0]))]
    if len(part_mat_a) == 0:
        return part_mat_b
    for i in range(len(part_mat_a)):  
        for j in range(len(part_mat_a[0])):
            #print("i", i, "j", j)
            res[i][j] = part_mat_a[i][j] + part_mat_b[i][j]
